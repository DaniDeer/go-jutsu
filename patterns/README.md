# Patterns

Deep-dive explanations of Go patterns and idioms.

Each pattern file includes:

- What it is (and what it isn't)
- Where you'll see it used
- Working examples
- Common gotchas

## Current Patterns

### Foundational (4)

- [Closure Pattern](./go-closure-currying-pattern.md)
- [Defer Cleanup Pattern](./defer-cleanup-pattern.md)
- [Empty Struct Signal Pattern](./empty-struct-signal-pattern.md)
- [Zero Values Pattern](./zero-values-pattern.md)

### Gotchas & Idioms (3)

- [Nil Interface Gotcha](./nil-interface-gotcha.md)
- [Comma-Ok Idiom](./comma-ok-idiom.md)
- [Blank Identifier Pattern](./blank-identifier-pattern.md)

### Concurrency (2)

- [Select Statement Pattern](./select-statement-pattern.md)
- [Context Propagation Pattern](./context-propagation-pattern.md)

### Advanced (5)

- [Functional Options Pattern](./functional-options-pattern.md)
- [Interface Satisfaction Pattern](./interface-satisfaction-pattern.md)
- [Embedding Composition Pattern](./embedding-composition-pattern.md)
- [Method Values vs Expressions](./method-values-expressions-pattern.md)
- [Type Switch Pattern](./type-switch-pattern.md)

### Functional & Declarative (5)

- [Map/Filter/Reduce with Generics](./map-filter-reduce-generics.md)
- [Pipeline Pattern](./pipeline-pattern.md)
- [Function Composition Pattern](./function-composition-pattern.md)
- [Table-Driven Design](./table-driven-design-pattern.md)
- [Generator Pattern](./generator-pattern.md)
